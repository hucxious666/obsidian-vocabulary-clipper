...
async function sendNative(payload) {
  try {
    return await chrome.runtime.sendNativeMessage(HOST_NAME, payload);
  } catch (error) {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  sendNative(message.payload).then(async (response) => {
    if (response && response.ok && message.payload.action === "save_settings") await syncMenus();
